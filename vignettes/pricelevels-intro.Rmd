...
- CPD and NLCPD methods: `cpd()` and `nlcpd()`
- GEKS method: `geks()`
- Multilateral systems of equations: `gkhamis()`, `idb()`, `gerardi()`, and `rao()`
...
- Geary-Khamis method `gkhamis()` proposed by  @Geary1958 and @Khamis1972;
...
The *Gerardi index* and the *Iklé-Dikhanov-Balk (IDB) index* define the price levels exactly in the same way as the Geary-Khamis index. However, they define the international product prices differently. For the Gerardi index, the $\pi_n$'s are computed as simple geometric averages of the prices,
$$\pi_n = \left( \prod_{r=1}^{R} p_i^r \right)^{1/R} \quad n=1,\ldots,N \ , $$
while in the IDB index they rely on expenditure shares:
