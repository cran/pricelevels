...
- Multilateral variants of Carli, Dutot, Jevons, and Harmonic mean [@Rao2016]: `mcarli()`, `mdutot()`, `mjevons()`, `mharmonic()`
...
Following @Rao2016[p. 417], unweighted variants exist for these multilateral indices, which are implemented in

* `mcarli()` => unweighted variant of `rhajargasht()`
* `mdutot()` => `gkhamis()`
* `mjevons()` => `rao()`
* `mharmonic()` => `ikle()`
